...
class Player {
    public Player(APIContext ctx) { this.ctx = ctx; }
    protected IPlayerState state;
    protected APIContext ctx;

    public int health() { return ctx.localPlayer().getHealthPercent(); }
    public boolean inCombat() { return ctx.localPlayer().isInCombat(); }
    public Tile location() { return ctx.localPlayer().getLocation(); }
    public String update() { 
        state.update(this);
        return state.status();
    }
    public boolean isInteractingWith(NPC target) {
        return ctx.localPlayer().getInteracting() == target;
    }
}
